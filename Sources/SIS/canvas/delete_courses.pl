...
$client->GET("/api/v1/accounts/1/courses?access_token=$canvas_token");
my $response = from_json($client->responseContent());
print Dumper $response;
my $id = @$response[0]->{id};
print "ID: $id\n";

foreach my $course (@$response) {
    $client->DELETE("/api/v1/accounts/1/courses/$course->{id}?access_token=$canvas_token");
    my $delete_response = from_json($client->responseContent());
    print Dumper $delete_response;
}
