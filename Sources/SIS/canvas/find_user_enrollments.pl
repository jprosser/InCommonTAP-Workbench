...
$client->GET("/api/v1/accounts/1/users?access_token=$canvas_token&search_term=$user\@example.edu");
my $response = from_json($client->responseContent());
print Dumper $response;
my $id = @$response[0]->{id};

$client->GET("/api/v1/users/$id/enrollments?access_token=$canvas_token");
